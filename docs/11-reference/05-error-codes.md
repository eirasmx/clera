...
| Code | Level | Description |
|------|-------|-------------|
| `GESTURE_INVALID_ELEMENT` | warn | `app.gesture()` was called with something other than an `HTMLElement` as the target |
| `GESTURE_UNSUPPORTED` | warn | The gesture name passed to `app.gesture()` is not `"swipe"` |
| `GESTURE_NO_ACTIVE_PAGE` | warn | `app.gesture()` was called before any page has mounted |
