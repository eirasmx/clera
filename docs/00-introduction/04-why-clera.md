...
For large teams building complex products, this overhead is worth it. The tooling exists because the problems are real. Clera is not trying to replace every framework. It chooses a different tradeoff: less abstraction, less setup, and direct use of web standards. For many apps (internal tools, mobile WebView apps, prototypes, personal projects, anything that does not need a full component architecture) that tradeoff is a better fit than a full framework.
...
### 🔁 Developers who want one workflow across platforms

Developers who do not want a separate workflow for web, PWA, Android, and iOS. Clera keeps development centered on one application model and handles platform differences during packaging, using [Clera Studio](./02-the-clera-experience.md).

Tools like Capacitor and Cordova provide a bridge between an existing web app and a native shell. Clera covers the workflow before that step too: application structure, the runtime, live preview, debugging, and packaging all come from the same platform, not a wrapper added at the end.
...
[05 Runtime, Not Framework](./05-runtime-not-framework.md)
