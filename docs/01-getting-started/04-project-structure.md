...
This structure scales past a handful of pages. Once `index.html` itself gets long, the next step is splitting pages into their own files, covered next.
...
Combining page splitting with the css/js structure above is how a full sized app is organised in practice:
...
Nothing here is required by the runtime. This is the structure that a growing app tends to settle into, built entirely from the two mechanisms above: `<import>` for pages, and plain `<script src="...">` tags for JavaScript.
